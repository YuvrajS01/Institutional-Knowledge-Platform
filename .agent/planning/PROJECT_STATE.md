...
Phase 9 (Hardening) — P9-001/002/008 DONE; Phase 8 (Institutional AI) P0 DONE; Phase 6 — P6-003 DONE on task branch `feat/P6-003-summary-display`, P6-004/005/006/007 still TODO; Phase 4 — P4-004 DONE (merged #57 at `0fd561e`), P4-005 TODO; Phase 3 — P3-006/007 DONE (merged #53/#54), P3-009 DONE (merged #56), P3-010 TODO.

## Current Task

**P6-003** (Add document summary display — P1) — implementation complete on task branch `feat/P6-003-summary-display`; summary derived from extracted_text/metadata extra, 3 API tests passing, frontend card, typecheck/lint/build green, 519 tests total.

## Current Branch

`feat/P6-003-summary-display`

## Overall Status

`PHASE_9_DONE` — all P0 tasks DONE and `docs/FINAL_IMPLEMENTATION_REPORT.md` at `main` `0fd561e` (P4-004 merged); `PHASE_8_DONE` — all P0 DONE; `PHASE_6_PROGRESS` — P6-001/002 DONE, **P6-003 DONE on branch** (needs PR), P6-004/005/006/007 TODO; `PHASE_4_PROGRESS` — P4-001/002/003/006 DONE, **P4-004 DONE (merged #57)**, P4-005 TODO; `PHASE_3_PROGRESS` — P3-001..007/008 DONE, **P3-009 DONE (merged #56)**, P3-010 TODO.

## Last Completed Task

P6-003 (Add document summary display — P1) — `apps/api/src/modules/documents/documents.service.ts` (`DocumentDetailView.summary: string | null` + `extractSummary` helper (first 2 sentences 500 cap, fallback 300) + `get()` prefers `metadata.extra.summary`/`extracted_metadata.summary` then `current_version.extracted_text` heuristic via `listByDocumentId`) + `apps/web/src/app/documents/[id]/page.tsx` (adds `summary` to `DocumentDetail` interface + `created_by`, summary card with AI-generated notice or “No summary yet” placeholder, removes P6-003 TODO placeholder in Actions) + `apps/api/src/modules/documents/document-summary.route.test.ts` 3 integration tests (derived from extracted_text, prefers metadata extra, null when no text) + `TASK_MANIFEST` P6-003 TODO→DONE; build `2.65kB` for `/documents/[id]`, 519 tests passing (+3), typecheck/lint green; pgvector on 5434.

## What Is Working

- Everything from Phases 0–2 + P3-001..P5-001 merged through #57 (`main` at `0fd561e` includes P4-004 approval queue + P3-009 processing-status + P5-008 reranker + P3-007 dates + P3-006 metadata LLM + P9-001/002/008).
...
- **P3-009 (merged #56)**: processing_status per-version + polling UI + retry.
- **P4-004 (merged #57)**: approval queue UI + reject endpoint.
- **NEW P6-003 (branch)**:
  - **`apps/api/src/modules/documents/documents.service.ts`**: Extends `DocumentDetailView` with `summary: string | null` + `extractSummary` (first 2 sentences 500 cap, fallback 300) + `get()` logic: prefer `metadata.extra.summary`/`extracted_metadata.summary` (LLM), else `current_version.extracted_text` heuristic via `versions.listByDocumentId`, else null; includes `created_by`.
  - **`apps/web/src/app/documents/[id]/page.tsx`**: Adds `summary` + `created_by` to `DocumentDetail`, renders summary card (`#f8fafc` with “AI-generated summary — review before relying” note) or placeholder (`#fffbeb` “No summary available yet. Processing may still be queued.”), removes P6-003 TODO text from Actions card.
  - **`apps/api/src/modules/documents/document-summary.route.test.ts`**: 3 integration (pgvector:pg17, S3): derived from extracted_text (2 sentences), prefers metadata extra, null when no text.
  - **Build**: `pnpm --filter web build` 11 routes (`/documents/[id]` 2.65kB from 2.49kB) green.
...
- Phases 6 remainder: important dates API/UI (P6-004 now unblocked by P3-007), bookmarks (P6-005), related (P6-006 now unblocked by P5-008), share (P6-007).
...
- PR for P6-003 requires human approval to merge into `main` (repository merge policy). PRs #56 (P3-009) and #57 (P4-004) already merged at `65be8d5`/`0fd561e`.
...
- Summary is derived deterministically (first 2 sentences) when LLM summary not yet stored in `metadata.extra.summary`; displayed with “AI-generated summary — review before relying” disclaimer per AI is not authoritative (AGENTS.md §11).
- Git uses task branches and pull requests; merging into `main` requires the repository's approval policy.

## Current Git State

`main` at `0fd561e` (Merge PR #57 P4-004). Task branch `feat/P6-003-summary-display` adds summary display (3 tests, frontend card), all checks green:

```text
lint ✅  typecheck ✅  tests ✅ (519, +3)  build ✅ (11 routes, /documents/[id] 2.65kB)  format ✅  migration ✅ (pgvector on 5434)
```
...
| Unit/integration tests | PASS (519) |
...
| Document detail page (is_current, superseded_by, versions, summary) | PASS (build 11/11, 2.65kB) |
...
After P6-003 merges, start **P6-004** (Add important dates API/UI — P1, now unblocked by P3-007) or **P5-011** (Add filters/facets — P1) or **P4-005** (Build version history UI — P1) or **P6-005** (Add bookmarks — P1).
